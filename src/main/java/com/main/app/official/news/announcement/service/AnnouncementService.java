package com.main.app.official.news.announcement.service;
...
import com.main.app.official.news.announcement.mapper.AnnouncementMapper;
...
public class AnnouncementService {
    ...
    public Page<AnnouncementDto> getBoardList(Pageable pageable, String searchType, String keyword) {
        List<AnnouncementDto> list = announcementMapper.selectBoardList(pageable, searchType, keyword);
        long total = announcementMapper.countBoardList(searchType, keyword);
        return PaginationUtil.toPage(list, pageable, total);
    }

    public AnnouncementDto getBoardDetail(String rqstNo, boolean increaseViewCount) {
        AnnouncementDto board = announcementMapper.selectBoardDetail(rqstNo, increaseViewCount);
        if (board != null) {
    ...
    public void saveBoard(AnnouncementRequest request, List<MultipartFile> files) {
        if (request.getRqstNo() == null || request.getRqstNo().isEmpty()) {
            ...
            if (request.getParentNo() != null && !request.getParentNo().isEmpty()) {
                AnnouncementDto parent = announcementMapper.selectBoardDetail(request.getParentNo(), false);
                if (parent != null) {
                    request.setGroupNo(parent.getGroupNo());
                    request.setDepth(parent.getDepth() + 1);
                    announcementMapper.updateReplyOrder(parent.getGroupNo(), parent.getOrderNo());
