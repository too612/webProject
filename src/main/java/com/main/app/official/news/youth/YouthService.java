...
public class YouthService {
    ...
    public Page<YouthDto> getBoardList(Pageable pageable, String searchType, String keyword) {
        ...
        return PaginationUtil.toPage(youthMapper.selectBoardList(params), pageable, youthMapper.countBoardList(params));
    }
    ...
    public YouthDto getBoardDetail(String rqstNo, boolean increaseViewCount) {
        ...
        if (increaseViewCount) youthMapper.updateReadCount(params);
    ...
    public void saveBoard(YouthRequest request, List<MultipartFile> files) {
        if (request.getRqstNo() == null || request.getRqstNo().isEmpty()) {
            ...
            if (request.getParentNo() != null && !request.getParentNo().isEmpty()) {
                ...
                YouthDto parent = youthMapper.selectBoardDetail(pp);
                if (parent != null) {
                    ...
                    youthMapper.updateReplyOrder(ro); request.setOrderNo(parent.getOrderNo() + 1);
                }
            } else { request.setGroupNo(request.getRqstNo()); request.setDepth(0); request.setOrderNo(0); }
            youthMapper.insertBoard(request);
        } else youthMapper.updateBoard(request);
        processFiles(request.getRqstNo(), files);
    }

    @Transactional public void updateBoard(YouthRequest request, List<MultipartFile> files) { youthMapper.updateBoard(request); processFiles(request.getRqstNo(), files); }
    @Transactional public void deleteBoard(String rqstNo) { youthMapper.deleteComments(rqstNo); youthMapper.deleteBoard(rqstNo); }
    public AttachmentDto getFile(Long fileId) { return fileService.getFile(fileId); }
    public List<CommentDto> getCommentList(String boardNo) { return youthMapper.selectCommentList(boardNo); }
    public void saveComment(CommentDto comment) { youthMapper.insertComment(comment); }
    public CommentDto getComment(Long commentId) { return youthMapper.selectCommentById(commentId.intValue()); }
    public void handleVote(Long commentId, String action, String previousVote) {
        int id = commentId.intValue();
        if (previousVote == null) { if ("like".equals(action)) youthMapper.increaseLike(id); else youthMapper.increaseDislike(id); return; }
        if (previousVote.equals(action)) { if ("like".equals(action)) youthMapper.decreaseLike(id); else youthMapper.decreaseDislike(id); return; }
        if ("like".equals(action)) { youthMapper.decreaseDislike(id); youthMapper.increaseLike(id); }
        else { youthMapper.decreaseLike(id); youthMapper.increaseDislike(id); }
    }
