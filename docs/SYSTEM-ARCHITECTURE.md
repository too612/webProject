...
- **auth**: 인증 유스케이스(login/register/logout/session)와 관련 상태(authStore)만 포함. `ProtectedRoute`, `Header`는 인증 정보를 사용해도 책임이 다르므로 각자의 위치 유지.
- **corp**: 기관 기본정보를 읽기 전용으로 조회한다. Footer 같은 전역 레이아웃은 이 값을 소비만 한다.
- **menu**: 메뉴 조회·상태 관리를 `menuApi / menuHook / menuModel / menuStore`로 분리. 중앙 공통 `api/menuApi.ts`는 제거하고 `common/menu/` 단일 경로만 사용.
...
## 13. 첨부파일 UI 스펙 (common/attachment)
