...
- **editor**: `common/editor/`이 단일 소스. Tiptap v2 기반 리치텍스트 에디터. 사용처는 `LazyEditor`로 lazy import한다.
...
| 도메인                | 백엔드        | 프런트엔드    | 비고                                                                                     |
| --------------------- | ------------- | ------------- | ---------------------------------------------------------------------------------------- |
| `common`              | 완료          | 완료          | auth/menu/chatbot/editor/attachment 응집 완료                                            |
...
| 메서드 | 경로                                  | 설명                    |
| ------ | ------------------------------------- | ----------------------- |
| POST   | `/api/common/files/upload?boardNo=`   | 파일 업로드 (multipart) |
| GET    | `/api/common/files?boardNo=`          | 파일 목록 조회          |
...
## 12. 첨부파일 UI 스펙 (common/attachment)
...
| 도메인                   | 용도                   | 연결 상태            |
| ------------------------ | ---------------------- | -------------------- |
| `official/about/pastor`  | 담임목사 프로필 이미지 | UI 연결(저장 미연결) |
| 게시판 (announcement 등) | 게시글 첨부파일        | 미연결 (예정)        |
