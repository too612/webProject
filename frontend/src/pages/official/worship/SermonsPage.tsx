...
const VIEW_PATH = '/worship/sermons/view';
const WRITE_PATH = '/worship/sermons/write';
...
export default function SermonsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('');
      try {
        const data = await boardApi.getBoardList(LIST_PATH, {
          page,
          searchType,
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-gray-400">등록된 설교가 없습니다.</td>
                </tr>
              )}
              {rows.map((post) => (
