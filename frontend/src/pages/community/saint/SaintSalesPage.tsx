...
export default function SaintSalesPage() {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    communityApi.saint.getSales({ page, size: 10, keyword: keyword || undefined })
      .then((res) => {
        if (!mounted) return;
        setRows(res.items ?? []);
  ...
  return (
    <section className="space-y-4">
      <div className="space-y-0.5">
        <h2 className="text-lg font-bold text-brand-dark">성도 나눔터</h2>
        <p className="text-sm text-gray-500">성도 간 물건 나눔 활동을 기록하고 공유합니다.</p>
      </div>
      ...
      <div className="bg-white rounded-panel shadow-panel border border-gray-100 overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-brand-primary text-white"><tr>
            ...
            <th className="px-4 py-3 text-left font-medium">물건</th>
            <th className="px-4 py-3 text-left font-medium">판매자</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">불러오는 중...</td></tr>
              : rows.length === 0 ? <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">등록된 항목이 없습니다.</td></tr>
                : rows.map((row, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-500">{totalElements - (page * 10) - i}</td>
                    <td className="px-4 py-3 text-gray-700">{String(row.item ?? '-')}</td>
                    <td className="px-4 py-3 text-gray-700">{String(row.seller ?? '-')}</td>
                    <td className="px-4 py-3 text-gray-700">{String(row.price ?? '-')}</td>
