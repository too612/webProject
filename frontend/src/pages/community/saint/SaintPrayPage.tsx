...
export default function SaintPrayPage() {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    communityApi.saint.getPray({ page, size: 10, keyword: keyword || undefined })
      .then((res) => {
        if (!mounted) return;
        setRows(res.items ?? []);
      ...
      <div className="bg-white rounded-panel shadow-panel border border-gray-100 overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-brand-primary text-white"><tr>
            ...
            <th className="px-4 py-3 text-left font-medium">기도 제목</th>
              ...
              : rows.length === 0 ? <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">등록된 항목이 없습니다.</td></tr>
                : rows.map((row, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-700">{String(row.request ?? '-')}</td>
