import { COMMUNITY_SAINT_PRAY_COLUMNS, type CommunitySaintPrayRow } from './prayModel';
...
export default function SaintPrayPage() {
    const {
        ...
        handlePrevPage,
        handleNextPage,
    } = useCommunitySaintPrayPage();

    const getCellValue = (row: CommunitySaintPrayRow, key: string): string => {
        const value = row[key];
    ...
    return (
        <section className="space-y-4">
            <div className="space-y-0.5">
                <h2 className="text-lg font-bold text-brand-dark">성도 기도</h2>
                <p className="text-sm text-gray-500">성도 개인/가정 기도 요청을 공유합니다.</p>
            </div>
                ...
                <table className="w-full text-sm">
                    <thead className="bg-brand-primary text-white">
                        <tr>
                            ...
                            {COMMUNITY_SAINT_PRAY_COLUMNS.map((column) => (
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            <tr><td colSpan={COMMUNITY_SAINT_PRAY_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">불러오는 중...</td></tr>
                        ) : items.length === 0 ? (
                            <tr><td colSpan={COMMUNITY_SAINT_PRAY_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">등록된 항목이 없습니다.</td></tr>
                        ) : items.map((row, index) => (
