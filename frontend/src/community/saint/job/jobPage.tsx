import { COMMUNITY_SAINT_JOB_COLUMNS, type CommunitySaintJobRow } from './jobModel';
...
export default function SaintJobPage() {
    const {
        ...
        handlePrevPage,
        handleNextPage,
    } = useCommunitySaintJobPage();

    const getCellValue = (row: CommunitySaintJobRow, key: string): string => {
        const value = row[key];
    ...
    return (
        <section className="space-y-4">
            <div className="space-y-0.5">
                ...
                <p className="text-sm text-gray-500">채용 및 구직 정보를 커뮤니티 안에서 공유합니다.</p>
            </div>
                ...
                <table className="w-full text-sm">
                    <thead className="bg-brand-primary text-white">
                        <tr>
                            ...
                            {COMMUNITY_SAINT_JOB_COLUMNS.map((column) => (
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            <tr><td colSpan={COMMUNITY_SAINT_JOB_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">불러오는 중...</td></tr>
                        ) : items.length === 0 ? (
                            <tr><td colSpan={COMMUNITY_SAINT_JOB_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">등록된 항목이 없습니다.</td></tr>
                        ) : items.map((row, index) => (
