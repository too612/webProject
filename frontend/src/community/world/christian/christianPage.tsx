import { useCommunityWorldChristianPage } from './christianHook';
...
export default function WorldChristianPage() {
    ...
    } = useCommunityWorldChristianPage();
    ...
    return (
        <section className="space-y-5">
            ...
            <form className="flex gap-2" onSubmit={handleSearch}>
                <input
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/40 focus:border-brand-primary"
                        ...
                        items.map((item, index) => (
                            <article className="bg-white rounded-panel shadow-panel border border-gray-100 overflow-hidden" key={index}>
                                ...
                                <div className="p-4 space-y-2">
                                    ...
                                    <h3 className="font-semibold text-brand-dark text-sm">{String(item.title ?? '-')}</h3>
                                    <p className="text-xs text-gray-500 leading-relaxed">{String(item.summary ?? '-')}</p>
                                </div>
                            </article>
                        ))
                    )}
