...
const categoryCards: {
  ...
  },
  {
    ...
    desc: "소중한 순간을 공유해요",
    count: "145개의 글",
    to: "/community/facilities/calendar",
  },
];

export default function CommunityIndexPage() {
  const { indexData, loading, error } = useCommunityIndex();

  const communityStats: { icon: LucideIcon; value: string; label: string }[] = [
    {
      icon: Users,
      value: String(indexData.stats.totalMembers),
      label: "전체 회원",
    },
    {
      icon: FileText,
      value: String(indexData.stats.totalPosts),
      label: "전체 게시글",
    },
  ];
        ...
        <div className="container mx-auto px-4 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {categoryCards.map((card) => (
              <article
                className="bg-white rounded-panel shadow-panel border border-gray-100 p-5 flex flex-col gap-3 hover:shadow-card transition-shadow"
                key={card.title}
              >
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {loading ? (
              ...
              recentPosts.map((post) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="bg-brand-primary/10 text-brand-primary text-xs font-medium px-2.5 py-0.5 rounded-full">
                      {post.category}
                    </span>
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            <div className="bg-white rounded-panel shadow-panel border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <h3 className="font-semibold text-brand-dark flex items-center gap-1">
                  <Megaphone className="h-4 w-4" />
                  공지사항
                </h3>
                <Link
                  to="/community/group/manager"
              ...
              <ul className="divide-y divide-gray-100">
                {(indexData.notices.length > 0
                  ...
                  >
                    <Link
                      to="/community/group/manager"
                      className="flex items-center gap-1.5 text-sm text-gray-700 hover:text-brand-primary min-w-0 truncate"
                    >
                ...
                {(indexData.activities.length > 0
                  ...
                  >
                    <Link
                      ...
                      className="flex items-center gap-1.5 text-sm text-gray-700 hover:text-brand-primary min-w-0 truncate"
                    >
                      <span className="shrink-0 text-xs text-brand-primary font-medium">
                        [{item.category}]
                      </span>
                      <span className="truncate">{item.title}</span>
                    </Link>
            ...
            <div className="bg-white rounded-panel shadow-panel border border-gray-100 overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-px bg-gray-100">
                {communityStats.map((s) => (
                  <div
                    key={s.label}
                    className="bg-white flex flex-col items-center justify-center gap-1 p-5"
                  >
                    {(() => {
                      const Icon = s.icon;
                      return <Icon className="h-5 w-5 text-brand-primary" />;
