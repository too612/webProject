...
export default function FacilitiesCalendarPage() {
    const { year, month, events, loading, error, prevMonth, nextMonth } = useCommunityFacilitiesCalendarPage();
        ...
        <section className="space-y-4">
            <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                    <button type="button" onClick={prevMonth} className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm hover:bg-gray-50 transition-colors">
                        이전달
                    </button>
                    <span className="font-semibold text-brand-dark min-w-[7rem] text-center">{year}년 {month}월</span>
                    <button type="button" onClick={nextMonth} className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm hover:bg-gray-50 transition-colors">
                        다음달
                    </button>
                    <button type="button" className="bg-brand-primary text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-brand-primary/90 transition-colors">
                        일정 등록
                    </button>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                    {Array.from({ length: daysInMonth }, (_, index) => {
                        ...
                        return (
                            <article key={day} className="bg-white rounded-card shadow-card border border-gray-100 px-2 py-2 min-h-[4.5rem] flex flex-col">
