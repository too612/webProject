...
export default function FacilitiesDiningPage() {
    ...
    const getCellValue = (row: CommunityFacilitiesDiningRow, key: string): string => {
        const value = row[key];
    ...
    return (
        <section className="space-y-4">
            <div className="space-y-0.5">
                ...
                <p className="text-sm text-gray-500">식당 봉사 일정과 담당자 편성 정보를 공유합니다.</p>
            </div>
            ...
            <div className="bg-white rounded-panel shadow-panel border border-gray-100 overflow-x-auto">
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            <tr><td colSpan={COMMUNITY_FACILITIES_DINING_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">불러오는 중...</td></tr>
                        ) : items.length === 0 ? (
                            <tr><td colSpan={COMMUNITY_FACILITIES_DINING_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">등록된 항목이 없습니다.</td></tr>
                        ) : items.map((row, index) => (
