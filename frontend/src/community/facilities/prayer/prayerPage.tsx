import { COMMUNITY_FACILITIES_PRAYER_COLUMNS, type CommunityFacilitiesPrayerRow } from './prayerModel';
import { useCommunityFacilitiesPrayerPage } from './prayerHook';
...
export default function FacilitiesPrayerPage() {
    const {
        ...
        handlePrevPage,
        handleNextPage,
    } = useCommunityFacilitiesPrayerPage();

    const getCellValue = (row: CommunityFacilitiesPrayerRow, key: string): string => {
        const value = row[key];
                ...
                <table className="w-full text-sm">
                    <thead className="bg-brand-primary text-white">
                        <tr>
                            ...
                            {COMMUNITY_FACILITIES_PRAYER_COLUMNS.map((column) => (
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            <tr><td colSpan={COMMUNITY_FACILITIES_PRAYER_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">불러오는 중...</td></tr>
                        ) : items.length === 0 ? (
                            <tr><td colSpan={COMMUNITY_FACILITIES_PRAYER_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">등록된 항목이 없습니다.</td></tr>
                        ) : items.map((row, index) => (
