import { SYSTEM_LOG_LEVEL_CLASS, SYSTEM_LOG_SYSTEM_COLUMNS, type SystemLogSystemRow } from './systemModel';
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-start justify-between">
        ...
        <button type="button" className="inline-flex items-center gap-1 bg-brand-primary text-white rounded-lg px-4 py-2 text-sm font-semibold hover:bg-[#4e5caf] transition-colors">로그 내보내기</button>
      </div>

      <form className="flex gap-2" onSubmit={handleSearch}>
        <input className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary" placeholder="메시지 검색" value={inputKeyword} onChange={(event) => handleInputKeywordChange(event.target.value)} />
              ...
              <tr key={idx} className="hover:bg-gray-50 transition-colors">
                ...
                {SYSTEM_LOG_SYSTEM_COLUMNS.map((col) => (
                  <td key={String(col.key)} className="px-4 py-3 text-gray-700">
                    {col.key === 'level' ? (<span className={`community-badge ${SYSTEM_LOG_LEVEL_CLASS[getCellValue(row, 'level')] ?? ''}`}>{getCellValue(row, 'level')}</span>) : (getCellValue(row, String(col.key)))}
                  </td>
                ))}
              </tr>
            )))}
