...
export default function Header() {
  ...
  const topMenusFromApi = (() => {
    ...
    const sortedMenus = [...menuList].sort(
    ...
    const erpTopMenus = sortedMenus.filter((menu) =>
      erpTopIdPattern.test(String(menu.menuId ?? "")),
    );
    ...
    if (erpTopMenus.length > 0) {
      ...
      const erpChildrenByTopId = sortedMenus.reduce<Record<string, MenuItem[]>>(
        (acc, menu) => {
    ...
    const topLevelMenus = sortedMenus.filter(
      (menu) => menu.level === 1 || !menu.parentId,
    );

    // 일반 홈페이지 메뉴 구조의 부모 ID와 자식 소메뉴 리스트 간의 연결고리를 맺어주기 위한 객체 누적 적재 가동식입니다.
    const childrenByParent = sortedMenus.reduce<Record<string, MenuItem[]>>(
      (acc, menu) => {
  ...
  const familySiteOptions = [
    ...
    { value: "https://busan.psh.or.kr", label: "부산비전센터" },
    { value: "https://ilsan.psh.or.kr", label: "일산드림센터" },
  ...
  return (
    ...
    <header className="header sticky top-0 z-50 bg-white shadow-sm">
            ...
            {isAuthenticated ? (
              <>
                ...
                <button
                  type="button"
                  onClick={clearAuth}
                  className="hover:text-brand-primary transition-colors bg-transparent border-0 cursor-pointer text-sm text-inherit p-0"
                >
                  로그아웃
                </button>
              </>
      ...
      <div className="header-main border-b border-gray-200">
        ...
        <div
          className={`container max-w-[1200px] mx-auto px-5 flex items-center gap-4${isErpRoute ? " erp-wide max-w-full" : ""}`}
        >
          ...
          <nav
            ...
            style={
              isErpRoute
                ? ({ "--erp-menu-count": topMenus.length } as CSSProperties)
                : undefined
            }
          >
            ...
            {topMenus.map((menu) => {
              ...
              return (
                ...
                <div className="dropdown relative group" key={menu.menuId}>
                  ...
                  >
                    {menu.menuName}{" "}
                    {/* 가공 단을 거쳐 최종 수립된 메뉴 명칭 글씨(예: 공동체, 시스템 등)가 출력됩니다. */}
                  </Link>
      ...
      >
        ...
        <div className="mobile-nav-header">
          ...
          <Link to="/" className="mobile-logo" aria-label={`${corpName} 홈`}>
            <img
              src="/img/logo.png"
              alt={`${corpName} 로고`}
        ...
        <div className="mobile-nav-body">
          <ul className="mobile-menu-list">
            ...
            {topMenus.map((menu) => {
              ...
              if (hasSubmenu) {
                menuItemClassName = "has-submenu";
                if (isOpen) {
                  menuItemClassName += " submenu-open";
                }
              }
                ...
                <li key={menu.menuId} className={menuItemClassName}>
                  ...
                  <div className="menu-item-row">
                    ...
                    {hasSubmenu && (
                      <button
                        type="button"
                        className={`submenu-toggle-btn${isOpen ? " is-open" : ""}`}
            ...
            >
              ...
              {familySiteOptions.map((option) => (
                ...
                <option key={option.value || "default"} value={option.value}>
                  {option.label}{" "}
                  {/* 사용자 시야에 식별되는 직관적인 홈페이지 한글 매장 타이틀명(예: 두날개교회)이 드롭다운 내부에 각인됩니다. */}
                </option>
              ))}
            </select>
          </div>
