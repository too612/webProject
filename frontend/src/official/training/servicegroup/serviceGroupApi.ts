...
function isServiceMember(
  value: unknown,
): value is ServiceGroupContent["groups"][number]["members"][number] {
  if (!value || typeof value !== "object") return false;
  const c = value as Partial<
    ServiceGroupContent["groups"][number]["members"][number]
  >;
  return typeof c.name === "string";
}

function isServiceGroup(
  value: unknown,
): value is ServiceGroupContent["groups"][number] {
  if (!value || typeof value !== "object") return false;
  ...
  return (
    ...
    c.members.every((m) => isServiceMember(m))
  );
}
...
function isServiceGroupContent(value: unknown): value is ServiceGroupContent {
  ...
  return Array.isArray(c.groups) && c.groups.every((g) => isServiceGroup(g));
}

export const serviceGroupApi = {
  async getServiceGroupContent(): Promise<ServiceGroupContent | null> {
    try {
      const response = await client.get<ApiResponse<ServiceGroupContent>>(
        "/official/training/servicegroup",
      );
