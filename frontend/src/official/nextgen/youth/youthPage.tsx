...
type YouthBoardListProps = { title: string; breadcrumb: string; viewPath: string; writePath: string; listPath?: string; posts?: BoardItem[]; };
...
export default function YouthBoardListPage({ title, breadcrumb, viewPath, writePath, listPath, posts = [] }: YouthBoardListProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const data = await youthApi.getBoardList({ page, searchType, keyword: keyword.trim() || undefined });
        if (!mounted) return;
...
export function YouthViewPage() {
  const navigate = useNavigate(); const [searchParams] = useSearchParams(); const rqstNo = searchParams.get('rqstNo') ?? ''; const listPath = YOUTH_PATHS.listPath;
  ...
  const loadData = async () => { if (!rqstNo) return; const data = await youthApi.getBoardView(rqstNo); setBoard(data.board); setComments(data.comments); };
  useEffect(() => {
    if (!rqstNo) { setLoading(false); return; }
    let m = true; (async () => { setLoading(true); setError(''); try { const d = await youthApi.getBoardView(rqstNo); if (!m) return; setBoard(d.board); setComments(d.comments); } catch { if (!m) return; setError('게시글을 불러오지 못했습니다.'); } finally { if (m) setLoading(false); } })();
    return () => { m = false; };
  }, [listPath, rqstNo]);
  ...
  const onConfirm = async () => {
    ...
    if (board?.password) { const ok = await youthApi.checkPassword(rqstNo, password); if (!ok) { alert('비밀번호가 올바르지 않습니다.'); return; } }
    if (action === 'edit') { navigate(`${listPath}/write?rqstNo=${rqstNo}`); return; }
    try { await youthApi.deleteBoard(rqstNo); alert('게시글이 삭제되었습니다.'); navigate(listPath); } catch { alert('삭제에 실패했습니다.'); }
  };
  const onVote = async (id: number | string, a: 'like' | 'dislike') => { try { const r = await youthApi.voteComment(id, a); if (!r) return; setComments((p) => p.map((c) => String(c.commentId) === String(id) ? { ...c, likes: r.likes, dislikes: r.dislikes } : c)); } catch {} };
  const onSubmitReply = async (parentCommentId: number | undefined, form: CommentFormState) => {
    ...
    try { setCommentLoading(true); await youthApi.saveComment({ boardNo: rqstNo, content: form.content, writer: form.writer, secret: form.secret ? 'Y' : 'N', spoiler: form.spoiler ? 'Y' : 'N', password: form.password, parentCommentId }); await loadData(); } catch { alert('댓글 저장에 실패했습니다.'); } finally { setCommentLoading(false); }
  };
  ...
  return (
    <section><article className="rounded-panel border border-gray-100 bg-white p-6 shadow-panel">
      ...
      <section className="mt-6 border-t border-gray-100 pt-6"><div className="mb-3 flex items-center justify-between"><h3 className="m-0 text-base font-semibold"><span className="material-icons align-middle">chat_bubble</span> 댓글 {totalCC}</h3><div className="flex gap-1"><button type="button" className="rounded border px-2 py-1 text-xs" onClick={() => setSortType('latest')}>Latest</button><button type="button" className="rounded border px-2 py-1 text-xs" onClick={() => setSortType('popular')}>Popular</button></div></div>
        <div className="space-y-3">{topLevelComments.length === 0 && !loading && <div className="py-6 text-center text-sm text-gray-400">댓글이 없습니다.</div>}{topLevelComments.map((comment, index) => (<YouthCommentItem key={`${comment.commentId ?? 'comment'}-${index}`} comment={comment} depth={0} onVote={onVote} onSubmitReply={onSubmitReply} commentLoading={commentLoading} />))}</div>
...
function YouthCommentItem({ comment, depth, onVote, onSubmitReply, commentLoading }: { comment: CommentDto; depth: number; onVote: (id: number | string, a: 'like' | 'dislike') => void; onSubmitReply: (pid: number | undefined, f: CommentFormState) => Promise<void>; commentLoading: boolean; }) {
  ...
  return (
    <div className={`rounded-lg bg-gray-50 p-4${depth > 0 ? ' border-l-2 border-gray-200' : ''}`} style={depth > 0 ? { marginLeft: `${depth * 24}px` } : {}}>
      ...
      {comment.replies?.map((reply, idx) => (<YouthCommentItem key={`${reply.commentId ?? 'r'}-${idx}`} comment={reply} depth={depth + 1} onVote={onVote} onSubmitReply={onSubmitReply} commentLoading={commentLoading} />))}
    </div>
  );
}
...
export function YouthWritePage() {
  ...
  const isEdit = Boolean(rqstNo); const isReply = Boolean(parentNo); const listPath = YOUTH_PATHS.listPath;
