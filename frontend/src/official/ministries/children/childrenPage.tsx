...
function ChildrenBoardListPage({
    title,
    breadcrumb,
    viewPath,
    writePath,
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            setError('');
            try {
                const data = await childrenApi.getBoardList({
                    page,
                    searchType,
...
export function ChildrenViewPage() {
    ...
    const loadData = async () => {
        if (!rqstNo) return;
        const data = await childrenApi.getBoardView(rqstNo);
        setBoard(data.board);
    ...
    const onConfirm = async () => {
        ...
        if (board?.password) {
            const isValid = await childrenApi.checkPassword(rqstNo, password);
            if (!isValid) {
        ...
        try {
            await childrenApi.deleteBoard(rqstNo);
            alert('게시글이 삭제되었습니다.');
            navigate(listPath);
        } catch {
            alert('삭제에 실패했습니다.');
    ...
    const onVote = async (commentId: number | string, voteAction: 'like' | 'dislike') => {
        try {
            const result = await childrenApi.voteComment(commentId, voteAction);
            if (!result) return;
            setComments((prev) =>
                prev.map((comment) => {
...
export function ChildrenWritePage() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setLoading(true);
            try {
                const data = isReply ? await childrenApi.getReplyForm(parentNo) : await childrenApi.getWriteForm(rqstNo);
    ...
    const onSubmit = async (event: FormEvent) => {
        ...
        try {
            ...
            if (isEdit) {
                await childrenApi.updateBoard(payload, files);
                alert(`${title} 수정이 완료되었습니다.`);
            } else {
                await childrenApi.saveBoard(payload, files);
...
export default function ChildrenPage() {
    return (
        <ChildrenBoardListPage
            title="어린이사역"
            breadcrumb="홈 > 사역 > 어린이사역"
            viewPath={CHILDREN_PATHS.viewPath}
            writePath={CHILDREN_PATHS.writePath}
        />
    );
}
