...
function YouthBoardListPage({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('');
      try {
        const data = await youthApi.getBoardList({
          page,
          searchType,
...
export function YouthViewPage() {
  ...
  const listPath = YOUTH_PATHS.listPath;
  ...
  const loadData = async () => {
    if (!rqstNo) return;
    const data = await youthApi.getBoardView(rqstNo);
    setBoard(data.board);
  ...
  const onVote = async (commentId: number | string, voteAction: 'like' | 'dislike') => {
    try {
      const result = await youthApi.voteComment(commentId, voteAction);
      if (!result) return;
      setComments((prev) =>
        prev.map((comment) => {
...
export function YouthWritePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const data = isReply ? await youthApi.getReplyForm(parentNo) : await youthApi.getWriteForm(rqstNo);
...
export default function YouthPage() {
  return (
    <YouthBoardListPage
      title="청년사역"
      breadcrumb="홈 > 사역 > 청년사역"
      viewPath={YOUTH_PATHS.viewPath}
      writePath={YOUTH_PATHS.writePath}
    />
  );
}
