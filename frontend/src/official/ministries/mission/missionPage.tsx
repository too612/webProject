...
import { MISSION_PATHS } from './missionModel';
...
function MissionBoardListPage({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('');
      try {
        const data = await missionApi.getBoardList({
          page,
          searchType,
...
export function MissionViewPage() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const rqstNo = searchParams.get('rqstNo') ?? '';
  const listPath = MISSION_PATHS.listPath;
  ...
  const loadData = async () => {
    if (!rqstNo) return;
    const data = await missionApi.getBoardView(rqstNo);
    setBoard(data.board);
  ...
  const onConfirm = async () => {
    ...
    if (board?.password) {
      const isValid = await missionApi.checkPassword(rqstNo, password);
      if (!isValid) {
    ...
    try {
      await missionApi.deleteBoard(rqstNo);
      alert('게시글이 삭제되었습니다.');
      navigate(listPath);
    } catch {
      alert('삭제에 실패했습니다.');
  ...
  const onVote = async (commentId: number | string, voteAction: 'like' | 'dislike') => {
    try {
      const result = await missionApi.voteComment(commentId, voteAction);
      if (!result) return;
      setComments((prev) =>
        prev.map((comment) => {
...
export function MissionWritePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const data = isReply ? await missionApi.getReplyForm(parentNo) : await missionApi.getWriteForm(rqstNo);
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        await missionApi.updateBoard(payload, files);
        alert(`${title} 수정이 완료되었습니다.`);
      } else {
        await missionApi.saveBoard(payload, files);
...
export default function MissionPage() {
  return (
    <MissionBoardListPage
      title="선교사역"
      breadcrumb="홈 > 사역 > 선교사역"
      viewPath={MISSION_PATHS.viewPath}
      writePath={MISSION_PATHS.writePath}
    />
  );
}
