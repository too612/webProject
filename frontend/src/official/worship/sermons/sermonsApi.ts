...
export const sermonsApi = {
  ...
  async getBoardList(query: BoardListQuery = {}) {
    const response = await client.get<ApiResponse<SpringPage<SermonItem>>>(
      SERMONS_API_BASE_PATH,
      {
        params: {
          page: query.page ?? 0,
          size: query.size ?? 10,
  ...
  async getBoardView(rqstNo: string, password?: string) {
    const response = await client.get<ApiResponse<BoardViewPayload>>(
      `${SERMONS_API_BASE_PATH}/view`,
      {
  ...
  async getWriteForm(rqstNo?: string) {
    const response = await client.get<ApiResponse<SermonItem>>(
  ...
  async saveBoard(payload: Partial<SermonItem>) {
    ...
    const response = await client.post<
      ApiResponse<{ basePath?: string; rqstNo?: string }>
    >(`${SERMONS_API_BASE_PATH}/write`, formData, {
  ...
  async checkPassword(rqstNo: string, password: string) {
    ...
    try {
      const response = await client.post<ApiResponse<BoardViewPayload>>(
        `${SERMONS_API_BASE_PATH}/view`,
        payload,
        {
          headers: {
