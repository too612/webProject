...
import { QNA_PATHS } from './qnaModel';
...
function QnaBoardListPage({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('');
      try {
        const data = await qnaApi.getBoardList({
          page,
          searchType,
...
export function QnaViewPage() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const rqstNo = searchParams.get('rqstNo') ?? '';
  const listPath = QNA_PATHS.listPath;
  ...
  const loadData = async () => {
    if (!rqstNo) return;
    const data = await qnaApi.getBoardView(rqstNo);
    setBoard(data.board);
  ...
  const onConfirm = async () => {
    ...
    if (board?.password) {
      const isValid = await qnaApi.checkPassword(rqstNo, password);
      if (!isValid) {
    ...
    try {
      await qnaApi.deleteBoard(rqstNo);
      alert('게시글이 삭제되었습니다.');
      navigate(listPath);
    } catch {
      alert('삭제에 실패했습니다.');
  ...
  const onVote = async (commentId: number | string, voteAction: 'like' | 'dislike') => {
    try {
      const result = await qnaApi.voteComment(commentId, voteAction);
      if (!result) return;
      setComments((prev) =>
        prev.map((comment) => {
...
export function QnaWritePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const data = isReply
          ? await qnaApi.getReplyForm(parentNo)
          : await qnaApi.getWriteForm(rqstNo);
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        await qnaApi.updateBoard(payload, files);
        alert(`${title} 수정이 완료되었습니다.`);
      } else {
        await qnaApi.saveBoard(payload, files);
...
export default function QnaPage() {
  return (
    <QnaBoardListPage
      title="Q&A"
      breadcrumb="홈 > 안내 > Q&A"
      listPath={QNA_PATHS.listPath}
      viewPath={QNA_PATHS.viewPath}
      writePath={QNA_PATHS.writePath}
    />
  );
}
