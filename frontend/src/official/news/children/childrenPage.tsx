...
function ChildrenBoardListPage({ title, breadcrumb, viewPath, writePath, listPath, posts = [] }: ChildrenBoardListProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try { const data = await childrenApi.getBoardList({ page, searchType, keyword: keyword.trim() || undefined }); if (!mounted) return; setItems(data.items); setPageSize(data.size); setTotalPages(data.totalPages); setTotalElements(data.totalElements); }
      catch { if (!mounted) return; setError('게시글 목록을 불러오지 못했습니다.'); setItems([]); setTotalPages(0); setTotalElements(0); }
  ...
  return (<><section className="space-y-4">
    ...
    <form className="flex flex-col gap-2 sm:flex-row" onSubmit={onSearch}>
      <div className="flex flex-1 gap-2"><select className="rounded-lg border border-gray-200 px-3 py-2 text-sm" value={searchType} onChange={(e) => setSearchType(e.target.value)}><option value="title">제목</option><option value="rqstId">작성자</option><option value="cont">내용</option></select>
        <input type="text" className="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm" placeholder="검색어를 입력하세요." value={inputKeyword} onChange={(e) => setInputKeyword(e.target.value)} /></div>
...
export function ChildrenViewPage() {
  ...
  const listPath = CHILDREN_PATHS.listPath;
  ...
  useEffect(() => { if (!rqstNo) { setLoading(false); return; } let m = true; (async () => { setLoading(true); setError(''); try { const d = await childrenApi.getBoardView(rqstNo); if (!m) return; setBoard(d.board); setComments(d.comments); } catch { if (!m) return; setError('게시글을 불러오지 못했습니다.'); } finally { if (m) setLoading(false); } })(); return () => { m = false; }; }, [listPath, rqstNo]);
  const openPasswordModal = (a: 'edit' | 'delete') => { setAction(a); setPassword(''); setShowPasswordModal(true); };
  const onConfirm = async () => { if (!action || !rqstNo) { setShowPasswordModal(false); return; } if (board?.password) { const ok = await childrenApi.checkPassword(rqstNo, password); if (!ok) { alert('비밀번호가 올바르지 않습니다.'); return; } } if (action === 'edit') { navigate(`${listPath}/write?rqstNo=${rqstNo}`); return; } try { await childrenApi.deleteBoard(rqstNo); alert('게시글이 삭제되었습니다.'); navigate(listPath); } catch { alert('삭제에 실패했습니다.'); } };
  ...
  const onSubmitReply = async (parentCommentId: number | undefined, form: CommentFormState) => { if (!rqstNo) return; if (!form.writer.trim() || !form.content.trim() || !form.password.trim()) { alert('작성자, 내용, 비밀번호를 모두 입력해 주세요.'); return; } try { setCommentLoading(true); await childrenApi.saveComment({ boardNo: rqstNo, content: form.content, writer: form.writer, secret: form.secret ? 'Y' : 'N', spoiler: form.spoiler ? 'Y' : 'N', password: form.password, parentCommentId }); await loadData(); } catch { alert('댓글 저장에 실패했습니다.'); } finally { setCommentLoading(false); } };
