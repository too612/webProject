...
type RegistrationBoardListProps = {
    title: string;
    breadcrumb: string;
    viewPath: string;
    writePath: string;
...
function RegistrationBoardListPage({
    title,
    breadcrumb,
    viewPath,
    writePath,
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            setError('');
            try {
                const data = await registrationApi.getBoardList({
                    page,
                    searchType,
...
function RegistrationCommentItem({
    comment,
    depth,
    onVote,
    onSubmitReply,
...
export function RegistrationViewPage() {
    ...
    const loadData = async () => {
        if (!rqstNo) return;
        const data = await registrationApi.getBoardView(rqstNo);
        setBoard(data.board);
    ...
    const onConfirm = async () => {
        ...
        if (board?.password) {
            const isValid = await registrationApi.checkPassword(rqstNo, password);
            if (!isValid) {
        ...
        try {
            await registrationApi.deleteBoard(rqstNo);
            alert('Post deleted.');
            navigate(listPath);
        } catch {
            alert('Delete failed.');
    ...
    const onVote = async (commentId: number | string, voteAction: 'like' | 'dislike') => {
        try {
            const result = await registrationApi.voteComment(commentId, voteAction);
            if (!result) return;
            setComments((prev) =>
                prev.map((comment) => {
    ...
    const onSubmitReply = async (parentCommentId: number | undefined, form: CommentFormState) => {
        ...
        try {
            ...
            await registrationApi.saveComment({
                boardNo: rqstNo,
...
export function RegistrationWritePage() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                const data = isReply
                    ? await registrationApi.getReplyForm(parentNo)
                    : await registrationApi.getWriteForm(rqstNo);
    ...
    const onSubmit = async (event: FormEvent) => {
        ...
        try {
            ...
            if (isEdit) {
                await registrationApi.updateBoard(payload, files);
                alert(`${title} ??륁젟???袁⑥┷??뤿???щ빍??`);
            } else {
                await registrationApi.saveBoard(payload, files);
...
export default function RegistrationPage() {
    return (
        <RegistrationBoardListPage
            title="새가족등록"
            breadcrumb="홈 > 소식 > 새가족등록"
            viewPath={REGISTRATION_PATHS.viewPath}
            writePath={REGISTRATION_PATHS.writePath}
        />
    );
}
