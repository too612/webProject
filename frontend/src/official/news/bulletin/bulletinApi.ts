...
import { BULLETIN_API_BASE_PATH } from './bulletinModel';
...
export const bulletinApi = {
  async getBoardList(query: BoardListQuery = {}) {
    const response = await client.get<ApiResponse<SpringPage<BoardDto>>>(BULLETIN_API_BASE_PATH, {
      params: {
        page: query.page ?? 0,
  ...
  async getBoardView(rqstNo: string) {
    const response = await client.get<ApiResponse<BoardViewPayload>>(`${BULLETIN_API_BASE_PATH}/view`, {
      params: { rqstNo },
    });
  ...
  async getWriteForm(rqstNo?: string) {
    const response = await client.get<ApiResponse<BoardDto>>(`${BULLETIN_API_BASE_PATH}/write`, {
  ...
  async getReplyForm(parentNo: string) {
    const response = await client.get<ApiResponse<BoardDto>>(`${BULLETIN_API_BASE_PATH}/reply`, {
      params: { parentNo },
    });
    return response.data.data ?? null;
  },
  ...
  async saveComment(data: {
    ...
    const response = await client.post<ApiResponse<{ boardNo: string }>>(
      `${BULLETIN_API_BASE_PATH}/comment/write`,
      formData,
  ...
  async saveBoard(payload: Partial<BoardDto>, files: File[] = []) {
    ...
    const response = await client.post<ApiResponse<{ basePath?: string; rqstNo?: string }>>(`${BULLETIN_API_BASE_PATH}/write`, formData, {
      headers: {
  ...
  async updateBoard(payload: Partial<BoardDto>, files: File[] = []) {
    ...
    const response = await client.post<ApiResponse<{ rqstNo?: string }>>(`${BULLETIN_API_BASE_PATH}/update`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
  ...
  async deleteBoard(rqstNo: string) {
    ...
    await client.post(`${BULLETIN_API_BASE_PATH}/delete`, payload, {
      headers: {
  ...
  async checkPassword(rqstNo: string, password: string) {
    ...
    try {
      const response = await client.post<ApiResponse<BoardViewPayload>>(`${BULLETIN_API_BASE_PATH}/view`, payload, {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
      });
  ...
  async voteComment(commentId: number | string, action: 'like' | 'dislike') {
    const response = await client.post<ApiResponse<{ likes: number; dislikes: number; userVote: string }>>(
      `${BULLETIN_API_BASE_PATH}/comment/vote`,
