...
import { NOTICE_PATHS } from "./noticeModel";
import type {
  BoardItem as BoardDto,
  CommentItem as CommentDto,
  FileItem as FileDto,
} from "../../../common/board/board.types";
...
export default function NoticeBoardListPage({
  ...
  const resolvedListPath = useMemo(
    () => listPath ?? writePath.replace(/\/write$/, ""),
    [listPath, writePath],
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError("");
      try {
        const data = await noticeApi.getBoardList({
          page,
          searchType,
  ...
  const rows: BoardItem[] = !error
    ? items.map((item, idx) => ({
        id: item.rqstNo ?? "",
        rowNum: totalElements - page * pageSize - idx,
        title: item.title ?? "",
        author: item.rqstId ?? "-",
  ...
  const onSecretConfirm = async () => {
    navigate(
      `${viewPath}?rqstNo=${secretModal.rqstNo}&password=${encodeURIComponent(secretModal.password)}`,
    );
    setSecretModal({ show: false, rqstNo: "", password: "" });
  };
  const pageButtons =
    totalPages > 0 ? Array.from({ length: totalPages }, (_, i) => i) : [0];
      ...
      <section className="space-y-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="mt-0.5 text-sm text-gray-400">
              게시물: {totalElements || rows.length} | 페이지: {page + 1}/
        ...
        <div className="overflow-x-auto rounded-panel border border-gray-100 shadow-panel">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs uppercase text-gray-500">
              <tr>
                <th className="px-4 py-3 font-medium w-[6%]">번호</th>
                <th className="px-2 py-3 w-[4%]"></th>
                <th className="px-4 py-3 text-left font-medium w-[44%]">
                  제목
                </th>
                <th className="px-4 py-3 font-medium w-[13%]">작성자</th>
                <th className="px-4 py-3 font-medium w-[13%]">작성일</th>
                <th className="px-4 py-3 font-medium w-[8%]">조회수</th>
              ...
              {rows.map((post) => (
                ...
                >
                  ...
                  <td className="px-2 py-3 text-center">
                    {post.hasFile && (
                      <span className="material-icons text-[16px] text-[#777]">
                        attach_file
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    {post.secret ? (
                      ...
                      >
                        {post.depth > 0 && (
                          <span className="material-icons mr-1 text-[16px] align-middle">
                            subdirectory_arrow_right
                          </span>
                        )}
                        <span className="material-icons mr-[2px] text-[14px]">
                          lock
                        </span>
                        {post.title}
                        {post.commentCount > 0 && (
                          <span className="ml-1 text-[13px] text-red-500">
                            [{post.commentCount}]
                          </span>
                        )}
                      </button>
                      ...
                      <Link
                        to={`${viewPath}?rqstNo=${post.id}`}
                        className={`${post.depth > 0 ? " pl-5" : ""} text-brand-dark hover:text-brand-primary hover:underline`}
                      >
                        {post.depth > 0 && (
                          <span className="material-icons mr-1 text-[16px] align-middle">
                            subdirectory_arrow_right
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${post.answered ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                    >
                      {post.answered ? "완료" : "대기"}
                    </span>
                  </td>
        ...
        <div className="flex justify-center gap-1">
          {pageButtons.map((bp) => (
            <button
              key={bp}
              type="button"
              className={`h-8 w-8 rounded text-sm ${bp === page ? "bg-brand-primary text-white" : "text-gray-600 hover:bg-gray-100"}`}
              onClick={() => setPage(bp)}
            >
              {bp + 1}
            </button>
        ...
        <form className="flex flex-col gap-2 sm:flex-row" onSubmit={onSearch}>
          ...
          <div className="flex gap-2">
            ...
            <Link
              className="inline-flex items-center rounded-lg bg-brand-primary px-4 py-2 text-sm font-medium text-white hover:bg-[#4e5caf]"
              to={writePath}
            >
              글쓰기
            </Link>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            ...
            <input
              ...
              onChange={(e) =>
                setSecretModal((p) => ({ ...p, password: e.target.value }))
              }
              placeholder="비밀번호"
              onKeyDown={(e) => {
                if (e.key === "Enter") onSecretConfirm();
              }}
            />
...
export function NoticeViewPage() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const rqstNo = searchParams.get("rqstNo") ?? "";
  const listPath = NOTICE_PATHS.listPath;
  ...
  const loadData = async () => {
    if (!rqstNo) return;
    const data = await noticeApi.getBoardView(rqstNo);
    setBoard(data.board);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError("");
      try {
        const d = await noticeApi.getBoardView(rqstNo);
        if (!m) return;
        setBoard(d.board);
  ...
  const openPasswordModal = (a: "edit" | "delete") => {
    setAction(a);
    setPassword("");
  ...
  const onConfirm = async () => {
    ...
    if (board?.password) {
      const ok = await noticeApi.checkPassword(rqstNo, password);
      if (!ok) {
    ...
    try {
      await noticeApi.deleteBoard(rqstNo);
      alert("게시글이 삭제되었습니다.");
      navigate(listPath);
    } catch {
      alert("삭제에 실패했습니다.");
  ...
  const onVote = async (id: number | string, a: "like" | "dislike") => {
    try {
      const r = await noticeApi.voteComment(id, a);
      if (!r) return;
      setComments((p) =>
        p.map((c) =>
          String(c.commentId) === String(id)
            ? { ...c, likes: r.likes, dislikes: r.dislikes }
            : c,
        ),
      );
    } catch {}
  ...
  const sortedComments = [...comments].sort((a, b) =>
    sortType === "popular"
      ...
      : String(b.insDt ?? "").localeCompare(String(a.insDt ?? "")),
  );
  const topLevelComments = sortedComments.filter((c) => !c.parentCommentId);
  const fileList: FileDto[] = board?.fileList ?? [];
  const t = board?.title ?? "Post";
  const a = board?.rqstId ?? "-";
  const dt = board?.insDt
    ? String(board.insDt).replace("T", " ").slice(0, 16)
    : "-";
  const v = board?.views ?? 0;
  const ct = board?.cont ?? "";
  const totalCC = (function f(list: CommentDto[]): number {
    return list.reduce((s, c) => s + 1 + f(c.replies ?? []), 0);
  })(comments);
    ...
    <section>
      <article className="rounded-panel border border-gray-100 bg-white p-6 shadow-panel">
        <header className="mb-5 border-b border-gray-100 pb-5">
          <h2 className="text-xl font-bold text-brand-dark">{t}</h2>
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <span
              className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${totalCC > 0 ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
            >
              {totalCC > 0 ? "완료" : "대기"}
            </span>
          </div>
        ...
        <section
          className="prose max-w-none py-4 text-gray-700"
              ...
              {fileList.map((f, idx) => (
                ...
                >
                  ...
                  <a
                    href={
                      f.downloadUrl ??
                      `/api${listPath}/download?fileId=${f.fileId}`
                    }
                    download={f.orgFileNm ?? f.filename}
                    className="text-blue-600 hover:underline"
                  >
                    {f.orgFileNm ?? f.filename ?? `File ${idx + 1}`}
                  </a>
                  {f.fileSize > 0 && (
        ...
        <section className="mt-6 border-t border-gray-100 pt-6">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="m-0 text-base font-semibold">
              <span className="material-icons align-middle">chat_bubble</span>{" "}
              댓글 {totalCC}
            </h3>
          ...
          >
            <div className="flex flex-wrap gap-2">
              <input
                ...
                onChange={(e) =>
                  setCommentForm((p) => ({ ...p, writer: e.target.value }))
                }
              />
              <input
                className="rounded border border-gray-200 px-2 py-1 text-sm"
                ...
                onChange={(e) =>
                  setCommentForm((p) => ({ ...p, password: e.target.value }))
                }
              />
            </div>
            <textarea
            ...
            <div className="mt-2 flex items-center gap-2">
              <label className="inline-flex items-center gap-1 text-xs">
                <input
                  ...
                  onChange={(e) =>
                    setCommentForm((p) => ({ ...p, secret: e.target.checked }))
                  }
                />{" "}
                Secret
              </label>
              <label className="inline-flex items-center gap-1 text-xs">
                <input
                  ...
                  onChange={(e) =>
                    setCommentForm((p) => ({ ...p, spoiler: e.target.checked }))
                  }
                />{" "}
                Spoiler
              </label>
              <button
                ...
                className="ml-auto rounded bg-brand-primary px-4 py-2 text-sm text-white disabled:opacity-50"
              >
        ...
        <div className="mt-6 flex flex-wrap gap-2 border-t border-gray-100 pt-5">
          ...
          {rqstNo && (
            <button
              type="button"
              className="rounded-lg bg-gray-100 px-4 py-2 text-sm text-gray-700"
              onClick={() => navigate(`${listPath}/write?parentNo=${rqstNo}`)}
            >
              답글
            </button>
          )}
...
function NoticeCommentItem({
  ...
  comment: CommentDto;
  depth: number;
  onVote: (id: number | string, a: "like" | "dislike") => void;
  onSubmitReply: (
  ...
  const handleReplySubmit = async (event: FormEvent) => {
    ...
    setReplyForm((p) => ({ ...p, writer: "", content: "", password: "" }));
  };
  return (
    <div
    ...
    >
      <div className="mb-2 flex items-center gap-2">
        {depth > 0 && (
          <span className="material-icons text-[16px] text-[#888]">
            subdirectory_arrow_right
          </span>
        )}
        ...
        {isSecret && (
          <span className="material-icons text-[14px] text-[#888]">lock</span>
        )}
        {isSpoiler && (
          <span style={{ fontSize: "12px", color: "#f90" }}>스포일러</span>
        )}
      </div>
      {isSecret ? (
        <div className="text-sm italic text-gray-400">
          비밀 댓글입니다. 관리자만 볼 수 있습니다.
        </div>
      ) : isSpoiler && !spoilerVisible ? (
        <div
          className="cursor-pointer select-none text-sm text-gray-700 blur-[6px]"
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        >
          <span className="material-icons text-[16px] text-green-500">
            thumb_up
          </span>
          {comment.likes ?? 0}
        </button>
        <button
          ...
          className="inline-flex items-center gap-1 text-xs text-gray-600"
        >
          <span className="material-icons text-[16px] text-red-500">
            thumb_down
          </span>
          {comment.dislikes ?? 0}
        </button>
        ...
        >
          <div className="flex flex-wrap gap-2">
            <input
              ...
              onChange={(e) =>
                setReplyForm((p) => ({ ...p, writer: e.target.value }))
              }
            />
            <input
              className="rounded border border-gray-200 px-2 py-1 text-sm"
              ...
              onChange={(e) =>
                setReplyForm((p) => ({ ...p, password: e.target.value }))
              }
            />
          </div>
          <textarea
          ...
          <div className="mt-2 flex items-center gap-2">
            <label className="inline-flex items-center gap-1 text-xs">
              <input
                ...
                onChange={(e) =>
                  setReplyForm((p) => ({ ...p, secret: e.target.checked }))
                }
              />{" "}
              Secret
            </label>
            <label className="inline-flex items-center gap-1 text-xs">
              <input
                ...
                onChange={(e) =>
                  setReplyForm((p) => ({ ...p, spoiler: e.target.checked }))
                }
              />{" "}
              스포일러
            </label>
...
export function NoticeWritePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const data = isReply
          ? await noticeApi.getReplyForm(parentNo)
          : await noticeApi.getWriteForm(rqstNo);
        if (!m || !data) return;
        setForm({
          title: data.title ?? "",
          author: data.rqstId ?? "",
  ...
  const handleSubmit = async () => {
    ...
    if (!form.author.trim()) return alert("작성자를 입력해 주세요.");
    if (!form.content.trim()) return alert("내용을 입력해 주세요.");
    if (!form.password.trim()) return alert("비밀번호를 입력해 주세요.");
    ...
    try {
      ...
      const result = await noticeApi.saveBoard(payload, files);
      if (result?.rqstNo) navigate(`${listPath}/view?rqstNo=${result.rqstNo}`);
    ...
    <section className="space-y-4">
      ...
      <div className="rounded-panel border border-gray-100 bg-white p-6 shadow-panel space-y-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea
            className="rounded-lg border border-gray-200 px-3 py-2 text-sm min-h-[200px]"
            value={form.content}
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label className="text-sm text-gray-600">비밀글</label>
        </div>
        <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2 pt-2">
          <button
            className="rounded-lg bg-brand-primary px-4 py-2 text-sm text-white disabled:opacity-50"
