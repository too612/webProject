...
import { MANAGER_COLUMNS, type ManagerRow } from './managerModel';
...
export default function ManagerPage() {
  ...
  const getCellValue = (row: ManagerRow, key: string): string => {
    const value = row[key];
      ...
      <div className="overflow-x-auto rounded-panel border border-gray-100 shadow-panel">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={MANAGER_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">
                  Loading...
                </td>
              </tr>
            ) : items.length === 0 ? (
