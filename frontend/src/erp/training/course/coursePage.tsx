import { useTrainingCoursePage } from './courseHook';
import { TRAINING_COURSE_COLUMNS, type TrainingCourseRow } from './courseModel';
...
export default function TrainingCoursePage() {
  const {
    ...
    handlePrevPage,
    handleNextPage,
  } = useTrainingCoursePage();

  const getCellValue = (row: TrainingCourseRow, key: string): string => {
    const value = row[key];
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-bold text-brand-dark">Course List</h2>
          <p className="mt-0.5 text-sm text-gray-500">Manage course data.</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left font-medium text-gray-500">
              ...
              {TRAINING_COURSE_COLUMNS.map((col) => (
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={TRAINING_COURSE_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">
                  Loading...
                </td>
              </tr>
            ) : items.length === 0 ? (
