import { useFacilityInventoryPage } from "./inventoryHook";
import {
  FACILITY_INVENTORY_COLUMNS,
  type FacilityInventoryRow,
} from "./inventoryModel";
...
export default function FacilityInventoryPage() {
  const {
    ...
    handlePrevPage,
    handleNextPage,
  } = useFacilityInventoryPage();

  const getCellValue = (row: FacilityInventoryRow, key: string): string => {
    const value = row[key];
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td
                  colSpan={FACILITY_INVENTORY_COLUMNS.length + 1}
