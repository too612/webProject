import { useFacilityVehiclePage } from './vehicleHook';
import { FACILITY_VEHICLE_COLUMNS, type FacilityVehicleRow } from './vehicleModel';
...
export default function FacilityVehiclePage() {
  const {
    ...
    handlePrevPage,
    handleNextPage,
  } = useFacilityVehiclePage();

  const getCellValue = (row: FacilityVehicleRow, key: string): string => {
    const value = row[key];
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-bold text-brand-dark">Vehicle List</h2>
          <p className="mt-0.5 text-sm text-gray-500">Manage vehicle data.</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left font-medium text-gray-500">
              ...
              {FACILITY_VEHICLE_COLUMNS.map((col) => (
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={FACILITY_VEHICLE_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">
                  Loading...
                </td>
              </tr>
            ) : items.length === 0 ? (
