import { useEventParticipantPage } from './participantHook';
import { EVENT_PARTICIPANT_COLUMNS, type EventParticipantRow } from './participantModel';
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-bold text-brand-dark">Participant List</h2>
          <p className="mt-0.5 text-sm text-gray-500">Manage participant data.</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left font-medium text-gray-500">
              ...
              {EVENT_PARTICIPANT_COLUMNS.map((col) => (
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={EVENT_PARTICIPANT_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">
                  Loading...
                </td>
              </tr>
            ) : items.length === 0 ? (
