import { useSermonAttendancePage } from './attendanceHook';
import { SERMON_ATTENDANCE_COLUMNS, type SermonAttendanceRow } from './attendanceModel';
...
export default function SermonAttendancePage() {
  const {
    ...
    handlePrevPage,
    handleNextPage,
  } = useSermonAttendancePage();

  const getCellValue = (row: SermonAttendanceRow, key: string): string => {
    const value = row[key];
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left font-medium text-gray-500">
              ...
              {SERMON_ATTENDANCE_COLUMNS.map((col) => (
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={SERMON_ATTENDANCE_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">
                  Loading...
                </td>
              </tr>
            ) : items.length === 0 ? (
