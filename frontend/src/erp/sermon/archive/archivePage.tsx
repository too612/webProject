import { useSermonArchivePage } from './archiveHook';
import { SERMON_ARCHIVE_COLUMNS, type SermonArchiveRow } from './archiveModel';
...
export default function SermonArchivePage() {
  const {
    ...
    handlePrevPage,
    handleNextPage,
  } = useSermonArchivePage();

  const getCellValue = (row: SermonArchiveRow, key: string): string => {
    const value = row[key];
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-bold text-brand-dark">설교 아카이브 목록</h2>
          <p className="mt-0.5 text-sm text-gray-500">설교 아카이브 데이터를 조회하고 관리합니다.</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left font-medium text-gray-500">
              ...
              {SERMON_ARCHIVE_COLUMNS.map((col) => (
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={SERMON_ARCHIVE_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">
                  불러오는 중...
                </td>
              </tr>
            ) : items.length === 0 ? (
