import { useAdminMinutesPage } from './minutesHook';
import { ADMIN_MINUTES_COLUMNS, type AdminMinutesRow } from './minutesModel';
...
export default function AdminMinutesPage() {
  ...
  const getCellValue = (row: AdminMinutesRow, key: string): string => {
    const value = row[key];
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-bold text-brand-dark">Minutes List</h2>
          <p className="mt-0.5 text-sm text-gray-500">Manage minutes data.</p>
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-panel border border-gray-100 shadow-panel">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={ADMIN_MINUTES_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">
                  Loading...
                </td>
              </tr>
            ) : items.length === 0 ? (
