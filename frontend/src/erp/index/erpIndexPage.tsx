...
import { ERP_INDEX_SHORTCUTS, getTaskStatusStyle } from './erpIndexModel';
          ...
          <ul className="space-y-2">
            {(indexData.recentSermonTasks.length > 0
              ...
              <li key={task.id} className="flex items-center justify-between gap-2">
                ...
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 ${getTaskStatusStyle(task.status)}`}>{task.status}</span>
              </li>
            ))}
          </ul>
        </article>
