...
export function useAccountManagerPage() {
  ...
  const loadManagerList = useCallback(async (query: ManagerListQuery): Promise<ManagerListResult<AccountManagerRow>> => {
    return accountManagerApi.getManagerList(query);
  }, []);
  ...
  useEffect(() => {
    ...
    loadManagerList({ page, keyword: keyword || undefined })
      ...
      .catch((e) => {
        if (!mounted) return;
        const message = e instanceof Error ? e.message : '관리 목록을 불러오지 못했습니다.';
        setError(message);
      })
      .finally(() => {
