import { useAccountManagerPage } from './managerHook';
import { ACCOUNT_MANAGER_COLUMNS, type AccountManagerRow } from './managerModel';
...
export default function AccountManagerPage() {
  const {
    ...
    handlePrevPage,
    handleNextPage,
  } = useAccountManagerPage();

  const getCellValue = (row: AccountManagerRow, key: string): string => {
    const value = row[key];
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-bold text-brand-dark">Manager List</h2>
          <p className="mt-0.5 text-sm text-gray-500">Manage manager data.</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left font-medium text-gray-500">
              ...
              {ACCOUNT_MANAGER_COLUMNS.map((col) => (
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={ACCOUNT_MANAGER_COLUMNS.length + 1} className="px-4 py-8 text-center text-gray-400">
                  Loading...
                </td>
              </tr>
            ) : items.length === 0 ? (
